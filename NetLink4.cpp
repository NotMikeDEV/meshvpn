...
NetLink4::NetLink4(unsigned char RoutingTable)
{
	Socket = socket(AF_NETLINK, SOCK_DGRAM, NETLINK_ROUTE);
	struct sockaddr_nl nladdr = { 0 };
	nladdr.nl_family = AF_NETLINK;
	nladdr.nl_pid = 0;
	nladdr.nl_groups = RTMGRP_IPV4_ROUTE | RTMGRP_LINK;
...
void NetLink4::AttachServer(RouteServer4* Parent)
{
	this->Parent = Parent;
}
int NetLink4::GetHandle()
{
	return Socket;
}
struct Route4 NetLink4::ParseRoute(struct nlmsghdr *nlh)
{
...
void NetLink4::Parse(struct nlmsghdr *nh)
{
...
void NetLink4::AddRoute(struct Route4 Route)
{
	char buffer[1024] = { 0 };
	...
	memcpy(RTA_DATA(rta), &Route.Destination, sizeof(struct in_addr));
	hdr->nlmsg_len += rta->rta_len;

	rta = (struct rtattr *)(buffer + hdr->nlmsg_len);
	rta->rta_type = RTA_GATEWAY;
	rta->rta_len = RTA_LENGTH(sizeof(struct in_addr));
	memcpy(RTA_DATA(rta), &Route.Gateway, sizeof(struct in_addr));
	hdr->nlmsg_len += rta->rta_len;
...
void NetLink4::DeleteRoute(struct Route4 Route)
{
	char buffer[1024] = { 0 };
	struct nlmsghdr* hdr = (struct nlmsghdr*)buffer;
	hdr->nlmsg_len = sizeof(struct nlmsghdr);
